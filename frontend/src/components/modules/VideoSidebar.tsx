...
export default function VideoSidebar({ tasks, onRemix, params, setParams }: VideoSidebarProps) {
    ...
    const selectableModels = params.generationMode === "r2v"
        ? VIDEO_R2V_MODELS.filter((model) => isR2vImageBased(model.id))
        : VIDEO_I2V_MODELS;
    const currentModelConfig =
        selectableModels.find(m => m.id === params.model) ??
        selectableModels[0] ??
        VIDEO_I2V_MODELS.find(m => m.id === DEFAULT_I2V_MODEL_ID) ?? VIDEO_I2V_MODELS[0];
                            ...
                            <section className="space-y-4">
                                ...
                                <div>
                                    <label className="block text-xs text-text-secondary mb-2">
                                        {tm("modelLabel")}
                                        {params.generationMode === "r2v" && <span className="text-primary ml-2">参考图模型</span>}
                                    </label>
                                    <GroupedModelGrid
