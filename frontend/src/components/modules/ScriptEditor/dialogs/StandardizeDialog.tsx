...
export default function StandardizeDialog({ projectId, editor, onClose }: { projectId: string; editor: Editor; onClose: () => void }) {
  ...
  const run = async () => { if (!text.trim()) return; setBusy(true); try { setPreview((await scriptEditorApi.standardizeScript(projectId, text, skill, skillId, instruction)).standardized_text); } catch (e) { setPreview(`修改失败：${e instanceof Error ? e.message : '请检查 API 配置'}`); } finally { setBusy(false); } };
