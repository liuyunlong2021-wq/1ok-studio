...
function extractShotBlocks(editor: Editor | null): ShotBlockData[] {
  ...
  function traverse(node: EditorJsonNode) {
    if (node.type === 'shotBlock') {
      shotIndex++;
      shots.push({
        id: typeof node.attrs?.id === 'string' ? node.attrs.id : `shot-${shotIndex}`,
        shotNumber: shotIndex,
        description: extractText(node),
        duration: typeof node.attrs?.duration === 'number' ? node.attrs.duration : 3,
        status: (node.attrs?.status as ShotBlockData['status']) || 'draft',
        thumbnailUrl: typeof node.attrs?.thumbnailUrl === 'string' ? node.attrs.thumbnailUrl : undefined,
      });
    }
    if (node.content) {
              ...
              >
                ...
                <div className="px-3 py-2.5">
                  ...
                  <div className="mt-2 flex items-center gap-2">
                    ...
                    >
                      {t(`views.status${shot.status.charAt(0).toUpperCase() + shot.status.slice(1)}`)}
                    </span>
