...
export default function SceneNavigator({ editor }: SceneNavigatorProps) {
  ...
  useEffect(() => {
    try {
      ...
      if (stored) setSceneColors(JSON.parse(stored));
  ...
  const handleColorChange = useCallback((sceneId: string, color: string | null) => {
    setSceneColors((prev) => {
      const next = { ...prev, [sceneId]: color };
      try { window.localStorage.setItem(colorStorageKey, JSON.stringify(next)); } catch { /* ignore quota */ }
      return next;
    });
  }, []);
  ...
  const handleDrop = useCallback((_e: React.DragEvent, targetId: string) => {
    ...
    if (!editor || !sourceId || sourceId === targetId) return;
    const doc = editor.state.doc;
    const scenes: Array<{ id: string; from: number; to: number }> = [];
    doc.descendants((node, pos) => {
      if (node.type.name === 'sceneHeading') scenes.push({ id: node.attrs.id, from: pos, to: doc.content.size });
      return true;
    });
    scenes.forEach((scene, index) => { scene.to = scenes[index + 1]?.from ?? doc.content.size; });
    ...
    const tr = editor.state.tr.delete(source.from, source.to);
    const targetPos = source.from < target.from ? target.from - (source.to - source.from) : target.from;
    tr.insert(Math.max(0, targetPos), slice.content);
