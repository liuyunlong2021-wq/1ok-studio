...
export default function ConsistencyVault() {
    ...
    const handleUpdateDescription = async (assetId: string, type: string, description: string) => {
        ...
        try {
            await api.updateAssetDescription(currentProject.id, assetId, type, description);
            // Shared series/global assets are not fully expanded in the mutation response.
            // Reload the hydrated project so the asset library cannot disappear.
    ...
    const handleRewriteDescription = async (assetId: string, type: string, description: string, instruction: string) => {
        if (!currentProject) return null;
        try {
            const result = await api.rewriteAssetDescription(currentProject.id, assetId, type, description, instruction);
            const updatedProject = await api.getProject(currentProject.id);
            updateProject(currentProject.id, updatedProject);
            return result.description;
        } catch (error: any) {
            throw new Error(error?.response?.data?.detail || error?.message || "AI 修改描述失败");
        }
    };
        ...
        <div className="flex flex-col h-full text-foreground">
            ...
            <AnimatePresence>
                {selectedAsset && selectedAssetId && selectedAssetType && (
                    <CharacterWorkbench
                            asset={{ ...(selectedAsset as any), full_body_asset: (selectedAsset as any).full_body_asset || (selectedAsset as any).image_asset, full_body_image_url: (selectedAsset as any).full_body_image_url || (selectedAsset as any).image_url, full_body_prompt: (selectedAsset as any).full_body_prompt || (selectedAsset as any).image_prompt || "" }}
                            assetType={selectedAssetType as "character" | "scene" | "prop"}
                            onClose={() => {
...
function CharacterDetailModal({ asset, type, onClose, onUpdateDescription, onRewriteDescription, onGenerate, onGeneratePrompt, isGenerating, stylePrompt = "", styleNegativePrompt = "", onGenerateVideo, onDeleteVideo, isGeneratingVideo }: any) {
    ...
    const handleSelectVariant = async (variantId: string) => {
        if (!currentProject) return;
        try {
            await api.selectAssetVariant(currentProject.id, asset.id, type, variantId);
            updateProject(currentProject.id, await api.getProject(currentProject.id));
        } catch (error) {
    ...
    const handleDeleteVariant = async (variantId: string) => {
        if (!currentProject) return;
        try {
            await api.deleteAssetVariant(currentProject.id, asset.id, type, variantId);
            updateProject(currentProject.id, await api.getProject(currentProject.id));
        } catch (error) {
    ...
    const handleGenerateClick = (batchSize: number) => {
        onGenerate(applyStyle, "", batchSize, imagePrompt);
    };
    ...
    const handleRewriteDescription = async () => {
        ...
        setRewriteError("");
        try {
            const rewritten = await onRewriteDescription(description, instruction);
            if (rewritten) {
    ...
    return (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-overlay backdrop-blur-sm p-8">
            <motion.div
                ...
                className="bg-surface border border-glass-border rounded-2xl w-full max-w-7xl h-[82vh] max-h-[calc(100vh-2rem)] flex overflow-hidden shadow-lg"
            >
                {/* Left: Variant Selector */}
                <div className="w-[42%] bg-surface relative border-r border-glass-border flex flex-col overflow-hidden">
                    {/* Tab Switcher */}
                ...
                <div className="w-[58%] flex flex-col">
                    ...
                    <div className="p-6 border-b border-glass-border flex justify-between items-center bg-surface">
                        <h2 className="text-2xl font-bold text-foreground">{asset.name}<span className="ml-2 text-sm font-normal text-text-muted">资产详情</span></h2>
                    ...
                    <div className="flex-1 p-6 overflow-y-auto space-y-6">
                        ...
                        <div className="space-y-2">
                            <div className="flex justify-between items-center">
                                <label className="text-sm font-bold text-text-secondary">描述</label>
                                {!isEditing && <div className="flex gap-3"><button onClick={() => setIsEditing(true)} className="text-xs text-primary hover:underline">Edit</button><button onClick={() => setShowRewriteBar((v: boolean) => !v)} disabled={isRewritingDescription} className="text-xs text-primary hover:underline disabled:opacity-50">{isRewritingDescription ? "AI 修改中..." : "AI 修改"}</button></div>}
                            </div>
                            {isEditing ? (
                                <div className="space-y-2">
                                    <textarea
                                ...
                                <>
                                    <p className="text-sm text-text-secondary leading-relaxed bg-glass p-3 rounded-lg border border-transparent hover:border-glass-border transition-colors">{asset.description}</p>
                                    {asset.extracted_description && <p className="text-[0.6875rem] text-text-muted mt-2">提取原文：{asset.extracted_description}</p>}
                                </>
                            )}
                            {showRewriteBar && <div className="rounded-xl border border-primary/25 bg-glass p-3"><div className="flex items-center gap-2"><Sparkles size={14} className="shrink-0 text-primary" /><span className="shrink-0 text-xs text-primary">AI 修改描述</span><input value={rewriteInstruction} onChange={(e) => setRewriteInstruction(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") handleRewriteDescription(); }} placeholder="输入修改要求，例如：补充材质细节" className="min-w-0 flex-1 bg-transparent text-xs text-text-secondary outline-none placeholder:text-text-muted" disabled={isRewritingDescription} /><button type="button" onClick={() => setShowRewriteBar(false)} className="text-xs text-text-muted hover:text-text-secondary">取消</button><button type="button" onClick={handleRewriteDescription} disabled={isRewritingDescription} className="rounded-md bg-primary px-2.5 py-1 text-xs text-white disabled:opacity-50">{isRewritingDescription ? "生成中" : "生成修改"}</button></div><div className="mt-2 flex flex-wrap gap-2">{["补充环境细节", "优化为生图描述", "只保留视觉信息"].map((item: string) => <button key={item} type="button" onClick={() => setRewriteInstruction(item)} className="rounded-md border border-glass-border px-2 py-1 text-[0.6875rem] text-text-muted hover:border-primary/40 hover:text-primary">{item}</button>)}</div>{rewriteError && <p className="mt-2 text-xs text-red-400">{rewriteError}</p>}</div>}
                        </div>
                        ...
                        {activeTab === "image" && (
                            <div className="space-y-2">
                                <div className="flex items-center justify-between">
                                    <label className="text-sm font-bold text-text-secondary uppercase">Image Prompt</label>
                                    <button
                                        type="button"
                                ...
                                <textarea value={imagePrompt} onChange={(e) => setImagePrompt(e.target.value)} className="w-full h-24 bg-input-bg border border-glass-border rounded-lg p-3 text-sm text-text-secondary resize-none focus:border-primary focus:outline-none" placeholder="Describe the asset..." />
                                <div className="flex justify-end"><button type="button" onClick={() => handleGenerateClick(1)} disabled={isGenerating} className="inline-flex items-center gap-1.5 rounded-lg bg-primary px-3 py-1.5 text-xs font-medium text-white hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-50"><RefreshCw size={13} className={isGenerating ? "animate-spin" : ""} />{isGenerating ? "生成中..." : "生成图片"}</button></div>
                            </div>
                        )}

                        {activeTab === "image" && (
                            <div className="space-y-2">
                                <label className="text-sm font-bold text-text-secondary">风格设置</label>
                    ...
                    <div className="p-6 border-t border-glass-border bg-surface flex gap-4">
                        <button
                            onClick={onClose}
                            className="flex-1 py-3 bg-primary hover:bg-primary/90 text-white rounded-xl font-bold flex items-center justify-center gap-2 shadow-sm shadow-primary/20"
                        >
                            <Check size={18} />
                            完成
                        </button>
