  ...
  return (
    <div className="flex items-center gap-3 p-3 rounded-[14px] bg-surface-inset border border-border-subtle">
      <div className="group relative w-20 h-20 shrink-0 rounded-[12px] overflow-hidden bg-elevated border border-border-subtle">
        {isAudioPath(path) ? (
          <div className="w-full h-full flex items-center justify-center bg-surface"><Music className="w-6 h-6 text-primary" /></div>
        ) : video ? (
          <video
...
export default function MediaInput() {
  ...
  const activeConfig = config ?? MODE_CONFIG.t2i!;

  const hasMedia = inputMedia.length > 0;
  const canAddMore = activeConfig.multiple && inputMedia.length < activeConfig.maxFiles;
    ...
    return (
      <div className="space-y-2">
        ...
        >
          ...
          <span className="text-[0.6875rem] text-text-muted">{t(`media.hints.${activeConfig.hintKey}`)}</span>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            {inputMedia.map((path, index) => (
              ...
              >
                {isAudioPath(path) ? (
                  <div className="w-full h-full flex items-center justify-center bg-surface"><Music className="w-5 h-5 text-text-muted" /></div>
                ) : isVideoPath(path) ? (
                  <video
                    src={resolveMediaSrc(path)}
                    className="w-full h-full object-cover"
