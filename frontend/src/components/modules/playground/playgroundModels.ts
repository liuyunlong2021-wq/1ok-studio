...
const AUDIO_MODES = new Set<string>(['t2a', 'r2a']);
...
function getFamilyPriority(family: string, mode: PlaygroundMode): number {
  if (AUDIO_MODES.has(mode)) return family === 'jiucaihezi' ? 0 : 999;
...
export function getModelsForMode(mode: PlaygroundMode): PlaygroundModelOption[] {
  if (AUDIO_MODES.has(mode)) {
    return [{ id: 'seed-audio-1.0', displayName: 'Seed Audio 1.0 · 韭菜盒子', family: 'jiucaihezi', description: '韭菜盒子音频生成', recommended: true, badges: ['韭菜盒子'], capabilities: ['t2a', 'r2a'], duration: null, params: {}, maxReferenceImages: 3 }];
  }
  return allModels
    .filter(([, model]) => {
