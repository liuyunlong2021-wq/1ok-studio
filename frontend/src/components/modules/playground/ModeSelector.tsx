...
const AUDIO_MODES: PlaygroundMode[] = ['t2a', 'r2a'];
...
export default function ModeSelector() {
  ...
  return (
    <div className="space-y-3">
      ...
      <div>
        <div className="mb-1.5 flex items-center gap-2">
          <span className="font-mono text-[0.5625rem] uppercase tracking-[0.18em] text-text-muted">{t('mode.groupAudio')}</span>
          <span className="h-px flex-1 bg-border-subtle atelier-group-line" />
        </div>
        <div className="flex gap-[2px] bg-surface-inset rounded-full p-[3px] atelier-pill-tabs">
          {AUDIO_MODES.map(renderPill)}
        </div>
      </div>
    </div>
  );
