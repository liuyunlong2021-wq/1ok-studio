...
export const scriptEditorApi = {
  ...
  updateScriptText: async (projectId: string, text: string): Promise<void> => {
    await axios.put(`${API_BASE}/projects/${projectId}/text`, { text });
  },

  listScriptSkills: async (kind: 'script' | 'motion' = 'script'): Promise<ScriptSkill[]> => (await axios.get(`${API_BASE}/script-skills`, { params: { kind } })).data,
  createScriptSkill: async (name: string, content: string, kind: 'script' | 'motion' = 'script'): Promise<ScriptSkill> => (await axios.post(`${API_BASE}/script-skills`, { name, content, kind })).data,
  updateScriptSkill: async (id: string, name: string, content: string, kind?: 'script' | 'motion'): Promise<ScriptSkill> => (await axios.put(`${API_BASE}/script-skills/${id}`, { name, content, kind })).data,
  deleteScriptSkill: async (id: string): Promise<void> => { await axios.delete(`${API_BASE}/script-skills/${id}`); },
  standardizeScript: async (projectId: string, text: string, skill: string, model?: string, skillId?: string, instruction?: string): Promise<StandardizeResponse> => {
    const res = await axios.post(`${API_BASE}/projects/${projectId}/standardize_script`, { text, skill, skill_id: skillId || '', model: model || '', instruction: instruction || '' });
    return res.data;
  },
