...
> 真相源：`logo-adaptation.html`（设计稿）+ `frontend/src/components/layout/OneOkBranding.tsx`（落地组件）。
...
**`OneOkBranding.tsx` 已提前完成改造**（工作区 modified，尚未 commit）。本清单从"待落地"转为"**落地验收 + 收尾**"。核验结论：组件实现与设计稿一致，仅剩资产提交、视觉巡检、暗色 logo 命名三处收尾。
...
> 对照 `OneOkBranding.tsx`：`LOGO_SRC` 映射(L13-19) + `ATELIER_DARK_FILTER`(L21) + `text-foreground`(L49) + `text-primary`(L52) —— **逐格匹配设计稿，✅**。
...
1. `git add public/logo-dark.png public/logo-light-teal.png public/logo-light.png`（连同 `OneOkBranding.tsx`）。
