...
DEFAULT_PROP_ASSET_PROMPT = """你是道具资产生图提示词专家。根据资产名称和描述，输出一段可直接用于图片生成的中文提示词。只描述形状、材质、细节和展示方式，不要解释，不要加标题。\n资产名称：{name}\n资产描述：{description}"""
DEFAULT_PROP_ASSET_PROMPT = """你是道具资产生图提示词专家。根据资产名称和描述，输出一段可直接用于图片生成的中文提示词。只描述形状、材质、细节和展示方式，不要解释，不要加标题。\n资产名称：{name}\n资产描述：{description}"""

# 全集声音导演稿。原文写死在 pipeline.generate_audio_plan_script 里，提到这里是为了
# 它能作为一个 prompt 阶段被 Skill 替换，同时还能在 UI 上显示「默认长什么样」。
DEFAULT_AUDIO_PLAN_PROMPT = (
    "你是影视声音导演。请把剧本整理成一份「全局声音导演稿」，用于后续一次性生成"
    "整集音频。\n"
    "稿子固定三段，每段的段名逐字写成「本片内容：」「角色音色：」「声音描述：」，各起一行；"
    "除这三个段名，不要再写别的标题、序号或 Markdown 标记。\n"
    "本片内容：一句话交代这是什么片子、这一段在演什么 —— 题材、时代、地点、事件，"
    "例如「汉末历史题材短剧 · 北方集市，两个汉子徒手角力」。它给下游模型定调，不念出来。\n"
    "角色音色：每个有台词的角色一行，写「角色名：年龄感、音色质感、语速、气质」；"
    "没有台词的角色不写。这段也是说明，不念出来。\n"
    "声音描述：按场景顺序写，每一段标明说话人、台词与情绪/语气提示，旁白单独成段；"
    "需要环境声或音效的地方用【】标注；台词要口语化、能直接念。"
    "要念、要演的内容只有这一段。\n"
    "要求：\n"
    "1. 全篇 ≤ 2500 个汉字（生成接口的输入上限是 3000 字符）。\n"
    "2. 不写打斗、血腥、伤害这类物理冲突细节 —— 上游 seed-audio 合成前会做内容审核，"
    "这类描写会被整篇拒收；冲突改用声音层表现：呼吸、衣料摩擦、脚步与地面、"
    "人群反应、环境声。"
)
...
# pipeline 写回中列时的 [:600]。
#
# 第 3 条不是洁癖：2026-09-19 实测，导演稿里一段肉搏描写会让整篇被上游文本审核拒收
# （`error.code=45001125` / `demo text audit failed`），逐词改写没用、只能整段换掉。
# 第 1 条的三段式（本片内容 / 角色音色 / 声音描述）是为了让上游看见「这是什么片」，
# 单看肢体描写容易被审核读歪。
AUDIO_PLAN_OUTPUT_CONTRACT = (
    "\n\n【输出硬要求，任何 Skill 都不得覆盖】\n"
    "1. 固定三段，段名逐字写成「本片内容：」「角色音色：」「声音描述：」各起一行；"
    "要念、要演的内容只在「声音描述：」一段，前两段是给人看的说明。\n"
    "2. 全篇 ≤ 2500 个汉字（下游 seed-audio-1.0 的输入上限是 3000 字符）。\n"
    "3. 不写打斗、血腥、伤害这类物理冲突细节 —— 上游 seed-audio 合成前会做内容审核，"
    "含这类描写的稿子会被整篇拒收；冲突改用声音层表现（呼吸、衣料摩擦、脚步与地面、"
    "人群反应、环境声）。\n"
    "4. 需要环境声或音效的地方用【】标注；台词口语化、可直接念，不要保留运镜与画面描述。"
)
...
{text}"""
    ...
    def rewrite_asset_description(self, asset_type: str, name: str, description: str, instruction: str, model: str = "") -> str:
        if not self.is_configured:
            raise ValueError("LLM API key 未配置")
        prompt = ("你是影视资产描述编辑器。只返回修改后的资产描述正文，不要标题、解释、Markdown、YAML 或 JSON。\n"
                  f"资产类型：{asset_type}\n资产名称：{name}\n当前描述：{description}\n"
                  f"修改要求：{instruction or '优化为清晰、可视化、适合资产制作的描述'}")
        return (self.llm.chat(messages=[{"role": "user", "content": prompt}], model=model or None) or "").strip()
    ...
    def analyze_to_storyboard(self, text: str, entities_json: Dict[str, Any], custom_extraction_prompt: str = "", model: str = "") -> List[Dict[str, Any]]:
        """
