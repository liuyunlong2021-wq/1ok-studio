...
BUILTIN_SCRIPT_SKILLS = [
    {"id": "builtin-short", "name": "中文短剧标准化", "content": "将输入内容转换为标准中文短剧剧本，保留剧情事实，不添加新情节。", "scope": "system", "is_builtin": True, "kind": "script"},
...
os.makedirs("output/playground/audio", exist_ok=True)
...
class StandardizeScriptRequest(BaseModel):
    text: str
    skill: str = ""
    skill_id: str = ""
    model: str = ""
...
class GenerateMotionPromptRequest(BaseModel):
    frame_ids: List[str]
    references: List[MotionPromptReference] = Field(default_factory=list)
...
async def standardize_script(script_id: str, request: StandardizeScriptRequest):
    ...
    try:
        result = await asyncio.get_event_loop().run_in_executor(
            None, lambda: LLMAdapter().chat([{"role": "user", "content": prompt}], model=model or None)
        )
        return {"standardized_text": result.strip(), "model": model or "system-default"}
    except Exception as e:
        raise HTTPException(status_code=502, detail=f"标准化失败: {e}")
...
class CreateVideoTaskRequest(BaseModel):
    ...
    reference_audio_urls: List[str] = []  # Reference audio URLs for MiniMax H3 (max 3)
...
class RewriteAssetDescriptionRequest(BaseModel):
    asset_id: str
    asset_type: str
    description: str
    instruction: str = ""
...
@app.post("/projects/{script_id}/assets/rewrite_description")
def rewrite_asset_description(script_id: str, request: RewriteAssetDescriptionRequest):
    script = pipeline.get_script(script_id)
    if not script:
        raise HTTPException(status_code=404, detail="Project not found")
    ...
    try:
        rewritten = pipeline.script_processor.rewrite_asset_description(
            request.asset_type, asset.name, request.description, request.instruction,
            pipeline.get_effective_polish_model(script),
        )
        if not rewritten:
            raise ValueError("模型返回空描述")
        updated = pipeline.update_asset_attributes(script_id, request.asset_id, request.asset_type, {
            "description": rewritten, "description_source": "ai", "description_updated_at": time.time(),
            "description_version": int(getattr(asset, "description_version", 1) or 1) + 1,
        })
        return {"description": rewritten, "source": "ai", "script": updated.model_dump()}
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
...
def _asset_contract(script_id: str, asset_type: str, asset_id: str) -> AssetContract:
    script = pipeline.get_script(script_id)
    if not script:
        raise HTTPException(status_code=404, detail="Project not found")
    asset, _ = pipeline._find_asset_with_source(script, asset_id, asset_type)
    if not asset:
        raise HTTPException(status_code=404, detail="Asset not found")
    if asset_type == "character":
        image_prompt = asset.full_body_prompt or ""
        media = {"full_body": asset.full_body_asset.model_dump(), "three_view": asset.three_view_asset.model_dump(), "headshot": asset.headshot_asset.model_dump()}
    else:
        image_prompt = getattr(asset.image_asset, "image_prompt", None) or ""
        media = {"image": asset.image_asset.model_dump()}
    return AssetContract(
        id=asset.id, asset_type=asset_type, name=asset.name, description=asset.description,
        extracted_description=getattr(asset, "extracted_description", None),
        description_source=getattr(asset, "description_source", "extracted"),
        description_updated_at=getattr(asset, "description_updated_at", 0.0),
        description_version=getattr(asset, "description_version", 1),
        image_prompt=image_prompt, media=media,
    )


@app.get("/projects/{script_id}/assets/{asset_type}/{asset_id}/contract", response_model=AssetContract)
def get_asset_contract(script_id: str, asset_type: str, asset_id: str):
    return _asset_contract(script_id, asset_type, asset_id)
...
def generate_asset_prompt(script_id: str, request: GenerateAssetPromptRequest):
    ...
    try:
        ...
        if request.asset_id:
            asset, _ = pipeline._find_asset_with_source(script, request.asset_id, request.asset_type)
            description_version = int(getattr(asset, "description_version", 1) or 1) if asset else 1
        return {"prompt": prompt, "source": "generated", "model": model or "default", "description_version": description_version}
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        raise HTTPException(status_code=502, detail=str(e))
...
def _get_polish_model_for_project(script_id: str) -> str:
    ...
    if not script:
        return ""
    return pipeline.get_effective_polish_model(script)
