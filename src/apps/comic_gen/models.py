...
class AssetContract(BaseModel):
    """Shared API contract for character, scene and prop workbenches."""
    id: str
    asset_type: str
    name: str
    description: str = ""
    extracted_description: Optional[str] = None
    description_source: str = "extracted"
    description_updated_at: float = 0.0
    image_prompt: str = ""
    image_prompt_source: str = ""
    image_prompt_model: str = ""
    image_prompt_description_version: int = 0
...
class VideoTask(BaseModel):
    ...
    reference_audio_urls: List[str] = Field(default_factory=list, description="Reference audio URLs for MiniMax H3 R2V (max 3)")
    source_frame_ids: List[str] = Field(default_factory=list, description="Ordered storyboard frame IDs represented by this video")
    skill_id: Optional[str] = Field(None, description="Motion Skill ID used to create the submitted prompt")
    skill_name: Optional[str] = Field(None, description="Motion Skill name snapshot used to create the submitted prompt")
...
class Character(BaseModel):
    ...
    extracted_description: Optional[str] = Field(None, description="Latest description extracted from the script")
...
class GlobalAssetLibrary(BaseModel):
    """Project-independent global asset pool (One OK Studio Core shared library).
